        ...
        for img, targets in pb(torch_dataset):
            ...
            if PRED_CLUSTERING:
                ...
                if "YOLO" in field_name:
                    ...
                    labels.append(torch.tensor([(box[5]+1) for box in np.array(preds[0].cpu())]))
                    scores.append(torch.tensor([box[4] for box in np.array(preds[0].cpu())]))
...
def evaluate(image, labels, preds, background_classes, iou_thresh, conf_thresh, show = True):
